...
def plot_training_curve(csv_path):
    ...
    ax1.scatter(best_epoch, df["val_loss"].iloc[best_epoch],
                s=80, zorder=5, label=f"best (epoch {best_epoch})")
...
def evaluate_model(model, test_ds, class_names):
    """
    Evaluate a binary image classification model and display results.

    Prints test loss and accuracy, a per-class classification report,
    and renders a confusion matrix heatmap.

    Assumes a binary model with a single sigmoid output unit.
    Predictions are thresholded at 0.5 (not argmax).
    ...
    test_ds : tf.data.Dataset
        Batched dataset yielding (images, labels) tuples. Labels should be
        binary integers (0 or 1).
    class_names : list of str
        Display names for the two classes, ordered by label index.
        Example: ["cat", "dog"]
    ...
    Example
    -------
    >>> evaluate_model(model, test_ds, class_names=["benign", "malignant"])
    """
    ...
    y_pred_classes = (y_pred.squeeze() >= 0.5).astype(int)
